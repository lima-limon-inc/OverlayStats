...
src_configure() {
	...
	econf ${fficonf} \
		...
		--libdir="${EPREFIX}"/usr/$(get_libdir)/${PN}-jdk3 \
		--datadir="${EPREFIX}"/usr/share \
		--bindir="${EPREFIX}"/usr/bin \
		--includedir="${EPREFIX}"/usr/include/${PN}3 \
...
src_install() {
	...
	local JDK_DIR="${EPREFIX}"/usr/${libdir}/${PN}-jdk3
	...
	local ecj_jar="${EPREFIX}"/usr/share/eclipse-ecj-4.2/lib/ecj.jar
	exeinto ${JDK_DIR}/bin
	sed -e "s#@JAVA@#/usr/bin/jamvm-2.0-3#" \
		-e "s#@ECJ_JAR@#${ecj_jar}#" \
		-e "s#@RT_JAR@#/usr/${libdir}/jamvm-jdk3/jre/lib/rt.jar#" \
		-e "s#@TOOLS_JAR@#/usr/${libdir}/jamvm-jdk3/lib/tools.jar#" \
	...
	pax-mark ${pax_markings} "${ED}"/usr/bin/jamvm-2.0-3
}
