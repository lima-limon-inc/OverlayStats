...
SLOT="0"
KEYWORDS="~amd64"
IUSE="autostart +desktop patch_compat"
...
PATCH_COMPAT_MAX_MAJOR=6
PATCH_COMPAT_MAX_MINOR=1

OFFICIAL_COMPAT_MAX_MAJOR=6
OFFICIAL_COMPAT_MAX_MINOR=0
...
src_prepare() {
	if kernel_is -gt $OFFICIAL_COMPAT_MAX_MAJOR $OFFICIAL_COMPAT_MAX_MINOR && use patch_compat; then
		eapply "${FILESDIR}/12.4.1__6_1__01_prandom_u32.patch"
		eapply "${FILESDIR}/12.4.1__6_1__02_snd_dma_continuous_data.patch"
	fi
	eapply_user
}
...
src_install() {
	...
	# NOTE: Not linking usr/lib/systemd as I do not use that and thus canot test it...
	symlinks=(
			'etc/init.d/DesktopVideoHelper'
			'etc/dracut.conf.d/dracut-blackmagic.conf'
