...
[[ -n "${SERVER_ROOT}" ]]     && declare -r SERVER_ROOT=${SERVER_ROOT}            || SERVER_ROOT="/var/lib/${game}"
[[ -n "${BACKUP_DEST}" ]]     && declare -r BACKUP_DEST=${BACKUP_DEST}            || BACKUP_DEST="/var/lib/${game}/backup"
...
[[ -n "${GAME_USER}" ]]       && declare -r GAME_USER=${GAME_USER}                || GAME_USER="fabric"
...
[[ -n "${SERVER_START_CMD}" ]] && declare -r SERVER_START_CMD=${SERVER_START_CMD} || SERVER_START_CMD="/usr/bin/java -Xms512M -Xmx1024M -jar './${MAIN_EXECUTABLE}' nogui"
...
# Check whether sudo or doas is needed at all
...
server_stop() {
	...
	if ${SUDO_CMD} screen -S "${SESSION_NAME}" -Q select . > /dev/null; then
		...
		if is_player_online; then
			...
			game_command say "Server is going down in 10 seconds..."
